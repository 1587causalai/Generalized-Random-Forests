...
\newcommand\todoitem[1]{}%{\textcolor{orange}{#1}}
...
我们这里观察如何从例子出发理解文章中的算法和理论。例如从Breidman森林，或者因果森林的例子出发。建立一个新的章节，收集 running examples. 
$$
\begin{cases}
...
\p{\htheta(x), \, \hnu(x)} \in \argmin_{\theta, \, \nu} \cb{\Norm{\sum_{i = 1}^n \alpha_i(x) \, \psi_{\theta, \, \nu}\p{O_i}}_2}.
\end{equation}
\footnote{ $\Norm{\sum_{i = 1}^n \alpha_i(x)  \psi_{\theta,  \nu}\p{O_i}}_2$ 只有两个自变量的函数， 如何得到 $\alpha_i(x)$ 是个最关键的问题。这里和变分推断类似，i.e. encoder $X_i \to \theta_i$ and decoder $\theta_i \to X_i$。} When the above expression has a unique root, we can simply say that \smash{$(\htheta(x), \, \hnu(x))$}
solves \smash{$\sum_{i = 1}^n \alpha_i(x) \, \psi_{\htheta(x), \, \hnu(x)}\p{O_i} = 0$}. The weights $\alpha_i(x)$ are traditionally obtained via ... 
...
\end{equation}
These weights sum to 1, and define the forest-based adaptive neighborhood of $x$; see Figure \ref{fig:weighting} for an illustration of this weighting function. 考虑两种极端情况，如果某个样本永远和(or not) $x$ 在同一片叶子，那么其权重是多少？总之，at a high level the estimates \smash{$\htheta(x)$} produced by a generalized random forests are simply obtained by solving \eqref{eq:gee_estimate} with weights \eqref{eq:forest_weights}.
...
We instead optimize an approximate criterion \smash{$\tDelta(C_1, \, C_2)$}
...
\ForAll{example $e \in \nn$}
\State $\alpha[{e}] \pluseq 1/\abs{\nn}$ \Comment{\parbox[t]{.44\linewidth}{i.e.  $\alpha_{bi}(x)= \frac{\1\p{\cb{X_i \in L_b(x)}}}{\abs{L_b(x)}}$}}
\EndFor
\EndFor
...
We now turn to a formal characterization of generalized random forests, with the aim of establishing asymptotic Gaussianity of the \smash{$\htheta(x)$}, and of providing tools for statistical inference about $\theta(x)$.
...
对于条件平均偏效应的估计问题 $(X_i, \, Y_i, \, W_i) \in \xx \times \RR \times \RR^q$, and posit a random effects model \smash{$Y_i = W_i \cdot b_i + \varepsilon_i$}, \smash{$\beta(x) = \EE{b_i \cond X_i = x}$}.
...
\begin{itemize}
    \item the $W_i$ are exogenous, i.e., independent of the unobservables conditionally on $X_i$: \smash{$\cb{b_i, \, \varepsilon_i} \indep W_i \cond X_i$}. 
    \item 其他条件 ...
\end{itemize}
